<?php
    ...
    function buylistview()
    {
        ...
        $config['base_url'] = base_url().'index.php/purchased/buylistview';
        $config['total_rows'] = $this->db->query("select count(item_id) as total from buylist_view")->result()[0]->total;
        $config['per_page'] = 60;
        $config['num_links'] = 8;
        ...
        $config['num_tag_close'] = '</li>';
        $this->pagination->initialize($config);
        $sql = "select myorder_item.*, buylist_view.times as total from buylist_view join myorder_item on (buylist_view.item_id = myorder_item.item_id) limit ? , ?";
        $data['records'] = $this->db->query($sql, array($this->uri->segment(3)===null?0:(int)($this->uri->segment(3)),$config['per_page']));
        
        $data['pageTag'] = 'buylistview';
        $data['main_containt'] = 'purchased/purchaseitems';
        $this->load->view('includes/template',$data);
    }
